...
# Applying the Factory Method Pattern for modularity and reusability
class DataLoader:
    @staticmethod
    def load_company_codes(file_path='issuers.csv'):
        company_codes = []
        try:
...
class PredictionModel:
    ...
    def train_model(self, X_train, y_train, target):
...
@app.route('/')
def home():
    company_codes = DataLoader.load_company_codes()
...
def company_data():
    ...
    historical_data = DataLoader.load_historical_data(company_code)
...
def technical_analysis():
    ...
    try:
        ...
        data['Target_1D'] = np.where(data['Future_Price_1D'] > data['last_price'], 1, np.where(data['Future_Price_1D'] < data['last_price'], -1, 0))
        data['Target_1W'] = np.where(data['Future_Price_1W'] > data['last_price'], 1, np.where(data['Future_Price_1W'] < data['last_price'], -1, 0))
        data['Target_1M'] = np.where(data['Future_Price_1M'] > data['last_price'], 1, np.where(data['Future_Price_1M'] < data['last_price'], -1, 0))

        data.dropna(subset=['Target_1D', 'Target_1W', 'Target_1M'], inplace=True)

        features = ['SMA_10', 'EMA_10', 'RSI', 'MACD', 'MACD_signal', 'MACD_hist', 'Stochastic_K', 'Stochastic_D', 'CCI', 'ROC']
        ...
        model.train_model(X_train, y_train_1D, '1D')
        model.train_model(X_train, y_train_1W, '1W')
        model.train_model(X_train, y_train_1M, '1M')

        latest_data = data[features].iloc[-1].values.reshape(1, -1)

        predicted_signal_1D = model.predict(latest_data, '1D')
        predicted_signal_1W = model.predict(latest_data, '1W')
        predicted_signal_1M = model.predict(latest_data, '1M')
